...
describe('TicketCard', () => {
  it('renders ticket information correctly', () => {
    renderWithDnd(<TicketCard issue={mockIssue} />);
  ...
  it('renders project color indicator', () => {
    const { container } = renderWithDnd(<TicketCard issue={mockIssue} />);
    
    // Colors are converted to RGB format in the DOM
    const projectIndicator = container.querySelector('[style*="rgb(59, 130, 246)"]');
  ...
  it('handles ticket without estimate', () => {
    ...
    renderWithDnd(<TicketCard issue={issueWithoutEstimate} />);
  ...
  it('handles ticket without project', () => {
    ...
    renderWithDnd(<TicketCard issue={issueWithoutProject} />);
  ...
  it('displays different priority levels correctly', () => {
    ...
    priorities.forEach(priority => {
      ...
      const { unmount } = renderWithDnd(<TicketCard issue={issueWithPriority} />);
  ...
  it('applies dragging styles when being dragged', () => {
    ...
    // The drag state is managed by react-dnd, so we test the structure is correct
    const ticketCard = container.firstChild as HTMLElement;
    expect(ticketCard).toHaveClass('cursor-grab');
    expect(ticketCard).toHaveClass('hover:shadow-md');
  });
  ...
  it('handles long titles gracefully', () => {
    ...
    renderWithDnd(<TicketCard issue={longTitleIssue} />);
  ...
  it('displays state with proper capitalization', () => {
    ...
    renderWithDnd(<TicketCard issue={issueWithLowerState} />);
